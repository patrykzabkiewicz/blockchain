...
char* sha256(char * toHash) {
	...
	char result[512] = {0};
	char buffer[64] = {0};
	_ltoa_s(H0, buffer, 16);
	strcat_s(result, 64, buffer);
	memset(buffer, 0, 64);
	_ltoa_s(H1, buffer, 16);
	strcat_s(result, 64, buffer);
	memset(buffer, 0, 64);
	_ltoa_s(H2, buffer, 16);
	strcat_s(result, 64, buffer);
	memset(buffer, 0, 64);
	_ltoa_s(H3, buffer, 16);
	strcat_s(result, 64, buffer);
	memset(buffer, 0, 64);
	_ltoa_s(H4, buffer, 16);
	strcat_s(result, 64, buffer);
	memset(buffer, 0, 64);
	_ltoa_s(H5, buffer, 16);
	strcat_s(result, 64, buffer);
	memset(buffer, 0, 64);
	_ltoa_s(H6, buffer, 16);
	strcat_s(result, 64, buffer);
	memset(buffer, 0, 64);
	_ltoa_s(H7, buffer, 16);
	strcat_s(result, 64, buffer);
...
{
    char stringToHash[50] = {0};

    if (argc < 2) {
        printf("too few arguments\n");
        printf("usage: sha256gen.exe stringToHash\n");
        return -1;
    }
    ...
    strcpy_s(stringToHash, 50, argv[1]);
