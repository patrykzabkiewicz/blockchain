...
#define HASH_LEN 65
#define TRANSACTION_CAPACITY 16
...
typedef struct _block {
	int idn;
	int tCount;
	Transaction t_list[TRANSACTION_CAPACITY];
	char hash[HASH_LEN];
	struct _block* next;
} Block;
...
void hashGenerate(char * prev, char * next) {
	strcpy_s(next, HASH_LEN, sha256(prev));
	strcpy_s(prev, HASH_LEN, next);
	return next;
}

void addBlock2Account(Account * acc, Block * bl) {}

void addTrasaction2Block(Transaction * t, Block * bl) {
	bl->t_list[bl->tCount] = *t;
	bl->tCount++;
}

void addHash2Block(char * hash, Block * bl) {
	strcpy_s(bl->hash, HASH_LEN, hash);
}

void printBlockChain(Block * chain) {
	int i;
	while (chain) {
		printf("\n\nBlock #%d: %s\n\n", chain->idn, chain->hash);

		for (i = 0; i < TRANSACTION_CAPACITY && i < chain->tCount; i++) {
			printf("idn: %d amount: %d from: %d to: %d\n", 
				chain->t_list[i].idn,
				chain->t_list[i].amount,
				chain->t_list[i].from,
				chain->t_list[i].to);
		}
...
Block* addToChain(Block* bl) {
	Block* newB = newBlock();
	newB->next = bl;
	return newB;
}

void removeFromChain(Block ** bl) {
	Block* tmp = (*bl)->next;
	free((*bl));
	*bl = tmp;
}
...
int main(int argc, char * argv[]) {
	...
	if (argc < 2) {
		...
		puts("usage: ./blockchain filename\n");
		return -1;
	}
	...
	err = fopen_s(&w, "hashindex.tt", "w");
	if (err) {
		perror("err: can't write to hash index file\n");
		_fcloseall();
		return -1;
	}
	...
	while(fileRowsCount > 0) {
		while ( fileRowsCount > 0 && i < TRANSACTION_CAPACITY)
		{
			fscanf_s(f, "%d %d %d %d", &t.idn, &t.amount, &t.from, &t.to);
			addTrasaction2Block(&t, chain);
			fileRowsCount--;
			i++;
		}
		i = 0;
		hashGenerate(prev, next);
		addHash2Block(next, chain);
		fprintf(w, "%d %s\n", chain->idn, next);
		chain = addToChain(chain);
	}

	removeFromChain(&chain);
